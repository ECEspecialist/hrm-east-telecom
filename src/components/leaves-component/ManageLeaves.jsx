...
import Button from "../../elements/editdeletebutton/EditDeleteButton"; 
...
function ManageLeave() {
  ...
  const [formData, setFormData] = useState({ id: null, employeeName: "", dateOfApplication: "", applicationType: "", duration: "", status: "pending" });
  const [leaves, setLeaves] = useState([
    { id: 1, employeeName: "John Doe", dateOfApplication: "2024-08-01", applicationType: "Sick Leave", duration: "3 days", status: "pending" },
    { id: 2, employeeName: "Jane Smith", dateOfApplication: "2024-07-20", applicationType: "Annual Leave", duration: "5 days", status: "approved" },
  ]);
  ...
  const openModal = (leave = null) => {
    if (leave) {
      ...
      setFormData({ id: null, employeeName: "", dateOfApplication: "", applicationType: "", duration: "", status: "pending" });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSave = () => {
    if (formData.id) {
      setLeaves((prev) =>
        prev.map((leave) =>
          leave.id === formData.id ? formData : leave
        )
      );
    } else {
      setLeaves((prev) => [
    ...
    <div className="manage-leave-container">
      ...
      <div className="display-information">
        <ul className="leaves-grid">
          ...
          {isAdmin && <li className="locate-center">Action</li>}
        </ul>
        {filteredLeaves.map((leave) => (
          <ul className="leaves-content" key={leave.id}>
            ...
            <li className={`separate-bar ${leave.status==="approved"?"approved-item-separate":leave.status==="pending"?"pending-item-separate":leave.status==="rejected"?"rejected-item-separate":null}`}>{leave.status}</li>
            {isAdmin && (
              <li>
                <Button
      ...
      >
        <h2 className="modal-title">{!formData.id ? "Create a New Leave Application" : "Edit Leave Application"}</h2>
